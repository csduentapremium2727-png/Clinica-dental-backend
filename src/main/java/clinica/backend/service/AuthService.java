...
public class AuthService {
    ...
    // 1. Autenticación normal de Spring Security
    ...
    // 2. BUSCAR EL USUARIO REAL EN BD PARA SACAR SU ID Y NOMBRES
    // (Esto es lo que faltaba: conectar el usuario técnico con tus datos de negocio)
    Usuario usuario = usuarioRepository.findByDocumentoIdentidad(dto.getDocumentoIdentidad())
            .orElseThrow(() -> new RuntimeException("Usuario no encontrado"));

    // 3. GENERAR EL TOKEN CON LOS DATOS EXTRA
    // Pasamos el ID, Nombre y Apellido al método modificado
    final String jwt = jwtUtil.generateToken(userDetails, usuario.getId(), usuario.getNombre(), usuario.getApellido());
