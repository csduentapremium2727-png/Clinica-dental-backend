...
public class JwtUtil {
    ...
    // Cambia la firma del método para aceptar los nuevos datos
    public String generateToken(UserDetails userDetails, Long id, String nombre, String apellido) {
        Map<String, Object> claims = new HashMap<>();
        
        // 1. Obtenemos el rol como antes
        String rol = userDetails.getAuthorities().stream()
                    .findFirst().orElseThrow().getAuthority();
        
        // 2. AGREGAMOS LOS DATOS QUE FALTABAN
        claims.put("rol", rol);
        claims.put("id", id);           // <--- ¡Aquí está la solución!
        claims.put("nombres", nombre);  // Para que salga el nombre en el front
        claims.put("apellidos", apellido); // Para que salga el apellido en el front
